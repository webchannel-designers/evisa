...
class Faqs_model extends CI_Model {
	...
	function get_pagination_count($cond='')
    {
		...
		if($this->session->userdata('faq_category_id')!=''){
			$this->db->where('category_id',$this->session->userdata('faq_category_id'));
		}
		if($this->session->userdata('faq_key')!=''){
			$this->db->like($this->session->userdata('faq_field'),$this->session->userdata('faq_key'),'both');
		}
        $this->db->from($this->table_name);
		$this->db->join($this->desc_table_name, "$this->desc_table_name.$this->foreign_key = $this->table_name.$this->primary_key");
		$this->db->where('language',$this->session->userdata('admin_language'));
