...
function get_subcategories3($cond)
	...
	function iscatexists($cond)

	{

		$this->db->where($cond);

		$this->db->from('brand_categories');
    ...
    {   
        $this->db->select('catdesc.name,brand_categories.*'); 
    ...
    function get_categories_array(  $branid='')
    ...
    { 
		...
		$this->db->join('products_category_desc catdesc', "cat.id =catdesc.content_category_id and language ='{$this->session->userdata('admin_language')}'",'left');
 		$query = $this->db->get();
        $this->db->order_by("brand_categories.category_order asc");
        return $query->result_array();
