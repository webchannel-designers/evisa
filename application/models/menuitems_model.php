...
class Menuitems_model extends CI_Model {
	...
	function get_menu_withsubmenu($menuid)
	{
		$this->db->distinct();
...
function get_subcategories2($cond)
	{
		$this->db->from($this->table_name);
		$this->db->join($this->desc_table_name, "$this->desc_table_name.$this->foreign_key = $this->table_name.$this->primary_key");
		$this->db->where('languag',$this->session->userdata('admin_language'));
	...
	{
		return array('news'=>'News','newslist'=>'News listing','attachments'=>'Attachments','contents'=>'Contents','contentlist'=>'Contents Listing','external'=>'External Link','internal'=>'Internal Link','nolink'=>'No Link');
	}
