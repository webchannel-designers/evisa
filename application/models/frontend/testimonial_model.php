...
class Testimonial_model extends CI_Model {
    function __construct()
    {
		...
		$this->desc_table_name='testimonial_desc';
		$this->primary_key ='id';
	...
	function get_array()
	{
		...
		$this->db->where('language',$this->session->userdata('front_language'));$this->db->order_by('sort_order','asc');
		$query = $this->db->get();
	...
	function get_subcategories3($cond)
	{
		$this->db->from($this->table_name);
		$this->db->join($this->desc_table_name, "$this->desc_table_name.$this->foreign_key = $this->table_name.$this->primary_key");
		$this->db->where('languag',$this->session->userdata('front_language'));
		$this->db->where($cond);$this->db->order_by('sort_order','asc');
		$query = $this->db->get();
        return $query->result_array();
	}
	...
	function get_categories_count($branid='')
    {
        $this->db->select('*');
        ...
        $this->db->where('id  in ($branid)',null,false);
        $this->db->from('products_category cat'); 
		$this->db->join('brand_categories', "brand_categories.category_id =cat.id",'left'); 
		$query = $this->db->get();
        return $query->num_rows();
    }
function get_categories_array($where='')
    { 
         //$this->db->select('cat.*,brand_categories.*,brand_categories.*');
	   // $this->db->where("cat.id  in ($brandcatid)",null,false);
        $this->db->where($where);
        $this->db->from('products_category cat'); 
		$this->db->join('brand_categories', "brand_categories.category_id =cat.id",'left'); 
		$this->db->join('products_category_desc catdesc', "cat.id =catdesc.content_category_id and language ='{$this->session->userdata('admin_language')}'",'left');
 		$query = $this->db->get();
        return $query->result_array();
    }
	function get_categories($num, $offset, $cond='')
    {
        $this->db->select('*');
		if(is_array($cond) && count($cond)>0){
		$this->db->where($cond);
