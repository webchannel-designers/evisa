...
class Blogscategory_model extends CI_Model {
	...
	function get_active()
	{
		...
		$this->db->join($this->desc_table_name, "$this->desc_table_name.$this->foreign_key = $this->table_name.$this->primary_key");
		$this->db->where('language',$this->session->userdata('front_language'));
		$this->db->order_by("sort_order ASC,$this->desc_table_name.name asc");
		$query = $this->db->get();
        return $query->result_array();
	}
	...
	function get_category_with_blogs($cond='')
	{
		if($cond) $this->db->where($cond);
		$this->db->select("count(blogs_category.`id`) as blogs,$this->desc_table_name.*,$this->table_name.*");
		$this->db->from($this->table_name);
		$this->db->join('blogs_desc', "blogs_desc.category	 = $this->table_name.$this->primary_key and blogs_desc.language = '{$this->session->userdata('front_language')}'");
		$this->db->join($this->desc_table_name, "$this->desc_table_name.$this->foreign_key = $this->table_name.$this->primary_key");
		$this->db->where("$this->table_name.status",'Y');
		$this->db->where("$this->desc_table_name.language",$this->session->userdata('front_language'));
		$this->db->group_by("$this->table_name.$this->primary_key");
		$this->db->order_by("$this->desc_table_name.name asc");
		$query = $this->db->get(); 
        return $query->result_array();
	}
