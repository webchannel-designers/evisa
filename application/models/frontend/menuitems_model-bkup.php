...
//	}
...

	{
		...
		if($count>0){
		$i=0;
		if($this->depth=='1' or $this->depth=='2') { $temp_tree .= "<div class='dropdown-menu'>"; }	
		$temp_tree .='<ul';
...
//					{
			...
			if($child['class']=="nav-item dropdown")
			{
				
				$tit=$child['name'];
				$class=" dropdown-toggle no-link";
			}
			else
			{
				$tit=$child['name'];
			...
			if($this->depth==''){$temp_tree .= ' class="nav-link'.$class.'"';}
			...
			if($this->depth=='0'){$temp_tree .= ' class="nav-link'.$class.'"';}
			$temp_tree .= ' target="'.$child['target_type'].'"';
			$temp_tree .='>'.$tit.'</a>';
			$this->depth++;
			$temp_tree .= $this->get_menu_ul($menuid,$child['id']);
			...
			$temp_tree .='</ul>';
			if($this->depth=='1' or $this->depth=='2') { $temp_tree .= "</div>"; }	
	...
	function get_menu_ul_top($menuid,$id='0')

	{
		...
		if($count>0){
		$i=0;
		if($this->depth=='1') { $temp_tree .= "<div class='dropdown-menu'>"; }	
		$temp_tree .='<ul';
		if($this->depth==''){$temp_tree .= " class='$menu->class'";} 
		$temp_tree .= '>';	
		//echo $this->depth;exit;
...
//			$temp_tree .='><a href="'. $link.'"';
//			if($child['class']=="search")
//			{
//				$tit="";
//				$class=" btn-search icon-search";
//			}
//			else
//			{
//				$tit=$child['name'];
//				$class="";
//			}
//			if($i==$count){ $temp_tree .=' class="req-app'.$class.' "';	}
//			if($link==current_url()){ $activeclass=" active"; } else { $activeclass=""; }
//			if($this->depth==''){$temp_tree .= ' class="nav-link'.$activeclass.$class.'"';}
//			$temp_tree .= ' target="'.$child['target_type'].'"';
//			$temp_tree .='>';
//			if($i==$count){ $temp_tree .='<i class="icon-clock"></i><span>';	}
//			$temp_tree .= $tit;
//			if($i==$count){ $temp_tree .='</span>';	}
//			$temp_tree .='</a>';
//			$this->depth++;
//			$temp_tree .= $this->get_menu_ul($menuid,$child['id']);
//			$this->depth--;
...
//				}
			...
			{
				$tit="";
				//$class=" nav-link dropdown-toggle no-link";
				$temp_tree .='<a class="nav-link dropdown-toggle no-link" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
			}
			else
			{
				$tit=$child['name'];
			...
			if($child['attachment']!="")
			{
			$temp_tree .='<i class="pr-2"><img class="img-fluid" src="'.base_url('public/uploads/menus/'.$child['attachment']).'" alt=""></i>';
			}
			$temp_tree .= $child['name'];
			//if($i==$count){ $temp_tree .='</span>';	}
			$temp_tree .='</a>';
			$this->depth++;
			$temp_tree .= $this->get_menu_ul_top($menuid,$child['id']);
			$this->depth--;
			array_push($this->exclude, $child['id']);
			$this->height++;
			$temp_tree .='</li>	';
