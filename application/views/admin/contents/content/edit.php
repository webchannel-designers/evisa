...
<div class="full_w">
	...
	<input id="id" name="id" type="hidden" value="<?php echo $content->id; ?>" />	
        ...
        <?php
		if($content->category_id==11)
		{
		?>
        <div class="element">

			<label for="location">Doctor name (<?php echo $this->languagesarr[$this->session->userdata('admin_language')]?>)<?php if(form_error('location')){ $err=' err'; echo form_error('location'); } else { $err=''; ?><span> (required)</span><?php } ?></label>
  ...
  <div class="element">
    ...
    <label for="image2">Image for home page (<?php echo $this->languagesarr[$this->session->userdata('admin_language')]?>) - <?php echo @$content->image3; ?></label>
    ...
    <input type="file" name="image3" />
		...
		<div class="entry">
			...
			<button type="submit" class="add">Save</button><a class="button cancel" href="<?php echo site_url('admin/contents/lists'); ?>">Cancel</a>
