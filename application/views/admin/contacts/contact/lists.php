...
<div class="full_w">
  ...
  <div class="entry">
    ...
    <div style="float:right;"> Sort :
      ...
      <select name="category" style="width:75px; margin-right:5px;">
        ...
        <option value="<?php echo $contactcat['id']; ?>" <?php if($this->session->userdata('contact_category_id')==$contactcat['id']){ echo 'selected="selected"'; }?>><?php echo $contactcat['name']; ?></option>
        <?php endforeach; ?>
      </select>
      Search :
      <input style="margin-right:5px;width:75px;" type="text" name="keyword" value="<?php echo $this->session->userdata('contact_key'); ?>" />
      <select name="field" style="width:75px;margin-right:5px;">
        <?php foreach($contactfields as $id => $key): ?>
        <option value="<?php echo $id; ?>" <?php if($this->session->userdata('contact_field')==$id){ echo 'selected="selected"'; }?>><?php echo $key; ?></option>
        <?php endforeach; ?>
      </select>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php echo substr(strip_tags($contact['location']),0,100); ?></td>
        <td><?php echo $contact['latitude']; ?></td>
        <td><?php echo $contact['longitude']; ?></td>
        ...
        <td><a href="<?php echo site_url('admin/contacts/edit/'.$contact['id'].'/'.$return); ?>" class="table-icon edit" title="Edit"></a> <a href="<?php echo site_url('admin/contacts/delete/'.$contact['id'].'/'.$return); ?>" class="table-icon delete" title="Delete" onclick="return confirmBox();"></a></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
  ...
  <div class="entry">
    ...
    <div class="sep"></div>
    <a class="button add" href="<?php echo site_url('admin/contacts/add'); ?>">Add New Contact</a> </div>
</div>
