<div class="full_w">
	...
	<?php echo form_open_multipart('admin/menus/menuitemedit/'.$menudetail->id.'/'.$menuitemdetail->id); ?>
		...
		<div class="element">
			...
			<label for="parent_id">Parent Menu <?php if(form_error('parent_id')){ $err=' err'; echo form_error('parent_id'); } else { $err=''; ?><span> (required)</span><?php } ?></label>
		...
		<div class="element">
			...
			<label for="show_subitems">Show Submenu <?php if(form_error('show_subitems')){ $err=' err'; echo form_error('show_subitems'); } else { $err=''; ?><span> (required)</span><?php } ?></label>

			<input type="radio" name="show_subitems" value="Y" <?php if($menuitemdetail->show_subitems=='Y'){ echo 'checked="checked"';} ?> /> Yes <input type="radio" name="show_subitems" value="N" <?php if($menuitemdetail->show_subitems=='N'){ echo 'checked="checked"';} ?> /> No
		...
		<div class="element">
			...
			<input type="radio" name="status" value="Y" <?php if($menuitemdetail->status=='Y'){ echo 'checked="checked"';} ?> /> Enabled <input type="radio" name="status" value="N" <?php if($menuitemdetail->status=='N'){ echo 'checked="checked"';} ?> /> Disabled

		</div>

		<div class="entry">

			<button type="submit" class="add">Save</button><a class="button cancel" href="<?php echo site_url('admin/menus/menuitems/'.$menudetail->id); ?>">Cancel</a>
