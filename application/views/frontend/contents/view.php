    ...
    <?php if($this->uri->segment(4)=='air-rep') { echo $content->desc; } else { ?>
    
    <section class="section" <?php if($this->uri->segment(4)=='who-we-are') { ?>id="<?php echo $content->slug ?>"<?php } ?>>
      <div class="container-fluid">
        <div class="row">
    ...
    <?php if($this->uri->segment(4)=='cruise') { if(count($offers)>0) { ?>
    
<section class="section" id="cruise">
      <div class="container-fluid">
      <!--<h4 class="col-blue1">Latest Offers</h4>-->
        <div class="row section-list off">
         <?php foreach($offers as $item) { ?>
    <div class="item col-xl-3 col-lg-4 col-sm-12 text-center mb-4">
		<a href="<?php echo base_url('public/uploads/products/'.$item['pdf']); ?>" target="_blank"><img alt="Regis latest offers" class="img-fluid img-thumbnail" src="<?php echo base_url('public/uploads/products/'.$item['image']); ?>"></a></div>
    <?php } ?>

     
  </div>
  
  <?php /*?><div class="pagination-wrap py-4"> 
