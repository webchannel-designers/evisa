...
class Downloads extends Web_Controller {
...

	{
		...
		$config['total_rows'] = $this->downloads_model->get_pagination_count(array('category_id'=>$pid));
		...
		$content['downloads']=$this->downloads_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),array('category_id'=>$pid));
...

	{
...

		{
			...
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$pid);
			...
			if($insertid){
				...
				redirect('admin/downloads/lists/'.$pid);
...

	{
...

		{
			...
			if($loginid){
				...
				redirect('admin/downloads/lists/'.$pid.'/'.$return);
