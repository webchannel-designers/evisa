...
class Insidebanner extends Web_Controller {
	function __construct()
    {
		...
		$this->load->model('insidegallery_model');	
		$this->load->model('insidegallerycategory_model');
	}
		
	public function index()
	{
		redirect('admin/insidebanner/lists');		
	}
	...
	public function lists()
	{
		...
		$config['base_url'] = site_url('admin/insidebanner/lists/');
		$config['total_rows'] = $this->insidegallery_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->insidegallery_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->insidegallerycategory_model->get_array();
		$content['galleries']=$this->insidegallery_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/insidebanner/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$add['categories']=$this->insidegallerycategory_model->get_array();
			$main['content']=$this->load->view('admin/insidebanner/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$descdata=array('title'=>$this->input->post('title'),'short_desc'=>$this->input->post('short_desc'),'image'=>$fileName);
			$insertid=$this->insidegallery_model->insert($maindata,$descdata);
...
//				$configSize1['source_image']    = 'public/uploads/insidegallery/'.$image;
//				$configSize1['maintain_ratio']  = TRUE;
//				$configSize1['width']           = 200;
//				$configSize1['height']          = 70;
//				$configSize1['master_dim'] = 'height';
//				$configSize1['new_image']   = 'public/uploads/insidegallery/thumb_'.$image;				
//				$this->image_lib->initialize($configSize1);
//				$this->image_lib->resize();
//				$this->image_lib->clear();
//				$configSize3['image_library']   = 'gd2';
//				$configSize3['source_image']    = 'public/uploads/insidegallery/'.$image;
//				$configSize3['maintain_ratio']  = TRUE;
//				$configSize3['width']           = 1600;
//				$configSize3['height']          = 1000;
//				$configSize3['new_image']   = 'public/uploads/insidegallery/main_'.$image;				
...
//			}			
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->insidegallery_model->load($id);
			$edit['categories']=$this->insidegallerycategory_model->get_array();
			$main['content']=$this->load->view('admin/insidebanner/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$this->input->post('category_id'));
	...
	function delete($id,$return)
	{
		$gallery= $this->insidegallery_model->load($id);
		if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->insidegallery_model->delete($id);
		if($loginid){
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			foreach($ids as $id):
				$loginid=$this->insidegallery_model->update($data,array(),$id);	
		...
		redirect('admin/insidebanner/lists/'.$this->input->post('return'));
	}
	...
	public function categories()
	{
		...
		$config['total_rows'] = $this->insidegallerycategory_model->get_pagination_count();
		$config['per_page'] = '10';
		$config['uri_segment'] = 4;
		$config['first_link'] = 'First';
		$config['last_link'] = 'Last';
		...
		$config['cur_tag_close'] = '</span>';
		$this->pagination->initialize($config);
		$content['galleries']=$this->insidegallerycategory_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/insidebanner/category/lists',$content,true);
		$this->load->view('admin/main',$main);
	}
	...
	function addcategory()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$main['content']=$this->load->view('admin/insidebanner/category/add','',true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			$config['upload_path'] = 'public/uploads/gallery';
			...
			$slug=$this->insidegallerycategory_model->create_slug($this->input->post('title'));
			$maindata=array('status'=>$this->input->post('status'),'slug'=>$slug);
			$descdata=array('title'=>$this->input->post('title'),'short_desc'=>$this->input->post('short_desc'),'image'=>$image);			
			$insertid=$this->insidegallerycategory_model->insert($maindata,$descdata);
			if($insertid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Gallery category added Successfully.</p></div>');
				redirect('admin/insidebanner/categories');
			} else {
	...
	function editcategory($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->insidegallerycategory_model->load($id);
			$main['content']=$this->load->view('admin/insidebanner/category/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			$slug=$this->insidegallerycategory_model->update_slug($this->input->post('slug'),$id);
			$maindata=array('status'=>$this->input->post('status'),'slug'=>$slug);
			$descdata=array('title'=>$this->input->post('title'),'short_desc'=>$this->input->post('short_desc'));
			$config['upload_path'] = 'public/uploads/gallery';
			$config['allowed_types'] = 'jpg|png|gif';
			...
			if($this->upload->do_upload('image'))
			{
				...
				$this->load->library('image_lib');
				$this->resizecrop($imagedata,'list_','296','183');
				$gallery= $this->insidegallerycategory_model->load($id);
				if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
			}
			$loginid=$this->insidegallerycategory_model->update($maindata,$descdata,$id);
			if($loginid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Gallery category updated Successfully.</p></div>');
				redirect('admin/insidebanner/categories/'.$return);
			} else {
	...
	function deletecategory($id,$return)
	{
		$gallery= $this->insidegallerycategory_model->load($id);
		if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->insidegallerycategory_model->delete($id);
		if($loginid){
	...
	function categoryactions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			foreach($ids as $id):
				...
				$loginid=$this->insidegallerycategory_model->update($data,array(),$id);				
			endforeach;
		}
		if(isset($_POST['sortsave']) && $this->input->post('sortsave')=='Save' && count($sort_orders)>0){
			foreach($sort_orders as $id => $sort_order):
		...
		redirect('admin/insidebanner/categories/'.$this->input->post('return'));
	}
