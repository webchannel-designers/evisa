...
class Team extends Web_Controller {
	...
	public function lists()
	{
		...
		$config['base_url'] = site_url('admin/team/lists/');
		$config['total_rows'] = $this->team_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->team_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->team_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/team/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/team/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$insertid=$this->team_model->insert($maindata,$descdata);
			if($insertid){
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->team_model->load($id);
			$edit['categories']=$this->teamcategory_model->get_array();
			$main['content']=$this->load->view('admin/team/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$this->input->post('category_id'));
	...
	function delete($id,$return)
	{
		$gallery= $this->team_model->load($id);
		if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->team_model->delete($id);
		if($loginid){
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			foreach($ids as $id):
				$loginid=$this->team_model->update($data,array(),$id);	
		...
		redirect('admin/team/lists/'.$this->input->post('return'));
	}
