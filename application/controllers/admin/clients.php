...
class Clients extends Web_Controller {
...

	{
		...
		$config['base_url'] = site_url('admin/clients/lists/');
		...
		$content['contentfields'] = $this->clients_model->get_fields();

		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');

		$content['contentcats'] = $this->newscategory_model->get_array();

		$content['contents']=$this->clients_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));

		$main['content']=$this->load->view('admin/clients/content/lists',$content,true);
...

	{
...

		{
			...
			$main['content']=$this->load->view('admin/clients/content/add',$add,true);
			...
			$slug=$this->clients_model->create_slug($this->input->post('title'));
			...
			$descdata=array('fname'=>$this->input->post('title'),'email'=>$this->input->post('author'),'phone'=>$this->input->post('phone'),'slug'=>$slug);

			$insertid=$this->clients_model->insert($maindata,$descdata);
...

	{
...

		{
			...
			$edit['content']= $this->clients_model->load($id);

			//$edit['widgets']=$this->widgets_model->get_rightwidgets();

			$main['content']=$this->load->view('admin/clients/content/edit',$edit,true);
			...
			$contentrow=$this->clients_model->load($id);
...
//			{
//			$slug=$this->clients_model->update_slug($this->input->post('title'),$id);
//			}
//			else
//			{
//			$slug=$this->clients_model->update_slug($this->input->post('slug'),$id);
//			}
			...
			$descdata=array('title'=>$this->input->post('title'),'fname'=>$this->input->post('fname'),'mname'=>$this->input->post('mname'),'lname'=>$this->input->post('lname'),'university'=>$this->input->post('university'),'department'=>$this->input->post('department'),'email'=>$this->input->post('author'),'address'=>$this->input->post('address'),'state'=>$this->input->post('state'),'city'=>$this->input->post('city'),'zip'=>$this->input->post('zip'),'country'=>$this->input->post('country'),'address2'=>$this->input->post('address2'),'state2'=>$this->input->post('state2'),'city2'=>$this->input->post('city2'),'zip2'=>$this->input->post('zip2'),'country2'=>$this->input->post('country2'),'phone'=>$this->input->post('phone'),'phone2'=>$this->input->post('mobile'),'degree'=>$this->input->post('degree'),'profession'=>$this->input->post('profession'),'organizations'=>$this->input->post('organizations'),'membership_type'=>$this->input->post('membership_type'));
			...
			$loginid=$this->clients_model->update($maindata,$descdata,$id);
...

	{
		...
		$content['content'] = $this->clients_model->get_row_cond($cond);				
...

	{
		...
		if(isset($_POST['sortsave']) && $this->input->post('sortsave')=='Save' && count($sort_orders)>0){
			...
			$loginid=$this->clients_model->update($data,array(),$contentid);			
