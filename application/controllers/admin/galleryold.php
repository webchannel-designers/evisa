...
class Galleryold extends Web_Controller {
...

    {
		...
		$this->load->model('galleryold_model');	
...

	{
		...
		{
			$edit['records']=$this->galleryold_model->get_array_cond($product_id);		
			$main['content']=$this->load->view('admin/galleryold/add',$edit,true);
			$this->load->view('admin/main',$main);
			...
			$insertid=$this->galleryold_model->insert($maindata,$descdata);
            }	
...
//			}			
			...
			if($insertid){
				...
				redirect('admin/galleryold/add/'.$product_id);
				...
				//$records=$this->galleryold_model->get_array();
...
//
//		{
//			$edit['records']=$this->galleryold_model->get_array();			
...
//			}			
			...
			$insertid=$this->galleryold_model->update($maindata,$descdata,$key['id']);
...

	{
		...
		$loginid=$this->galleryold_model->delete($id);
	...
	{
		//$gallerydata = $this->galleryold_model->load($id);
		$this->galleryold_model->updateall(array('is_default'=>'N'),array('parent_id'=>$reurn));
		$result = $this->galleryold_model->update(array(),array('is_default'=>$val),$id);
		//if($result){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Default image updated.</p></div>');

			redirect('admin/galleryold/add/'.$reurn);
