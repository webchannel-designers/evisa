...
class Brands extends Web_Controller {
	...
	public function lists()
	{
		...
		$config['base_url'] = site_url('admin/brands/lists/');
    ...
    public function viewcategories($brandid='')
	{
	 	$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Categories';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$brand = $this->testimonial_model->load($brandid); 
        $catid = $brand->category_id !='' ? $brand->category_id : '0';
        $cond = "cat.id  in ($catid) and brand_id = '$brandid'"  ;
		$config['base_url'] = site_url('admin/brands/viewcategories/');
		$config['total_rows'] = $this->testimonial_model->get_categories_count($cond);
		$config['per_page'] = 10;
		...
		$content['contents']=$this->testimonial_model->get_categories($config['per_page'],$this->uri->segment($config['uri_segment']),$cond);
		$main['content']=$this->load->view('admin/testimonial/content/viewcategories',$content,true);
		
		$this->load->view('admin/main',$main); 
	}
    
    function catactions($brandcatid=''){
         $brandid=$this->input->post('brandid');
       	$this->form_validation->set_rules('category_title[]', 'Title', 'trim');
       	$this->form_validation->set_rules('sort_order[]', 'Title', 'trim');
        if ($this->form_validation->run() == TRUE)
		{
        	if($this->input->post('category_title')){
                $sort_order = $this->input->post('sort_order');
                foreach($this->input->post('category_title') as $category => $title):
                    $slug = strtolower(url_title($title));
                    if($this->testimonial_model->iscatexists(array('category_id'=>$category,'brand_id'=>$brandid)))
                    { 
                        $this->db->update('brand_categories',array('category_title'=>$title,'catslug'=>$slug,'category_order'=>$sort_order[$category]),array('brand_id'=>$brandid,'category_id'=>$category));
                    }else{
                        $this->db->insert('brand_categories',array('brand_id'=>$brandid,'catslug'=>$slug,'category_id'=>$category,'category_title'=>$title,'category_order'=>$sort_order[$category]));
                    }
                endforeach;
            } 
            $this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p> Information saved  Successfully.</p></div>') ;
		}
		redirect('admin/brands/viewcategories/'.$brandid);
    }
	...
	function add()
	{
		...
		$this->form_validation->set_rules('video_desc', 'video_desc', '');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$config['allowed_types'] = 'doc|docx|xls|xlsx|ppt|pptx|jpg|gif|png';
            ...
            $category=implode(",",$this->input->post('category_id'));
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$category,'slug'=>$slug,'widgets'=>$widgets);
			$descdata=array('title'=>$this->input->post('title'),'author'=>$this->input->post('author'),'desc'=>$this->input->post('desc'),'video_desc'=>$this->input->post('video_desc'),'short_desc'=>$this->input->post('short_desc'),'image'=>$content_image,'banner_image'=>$banner_image,'date_time'=>$date_time);
			$insertid=$this->testimonial_model->insert($maindata,$descdata);
            if($this->input->post('category_id')){
                foreach($this->input->post('category_id') as $category):if($category>0){
                    $this->db->insert('brand_categories',array('brand_id'=>$insertid,'category_id'=>$category,'category_title'=>$this->input->post('category_title')));
               } endforeach;
            } 
			if($insertid){
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$category=implode(",",$this->input->post('category_id'));
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$category,'slug'=>$slug,'widgets'=>'');
			$descdata=array('title'=>$this->input->post('title'),'author'=>$this->input->post('author'),'desc'=>$this->input->post('desc'),'video_desc'=>$this->input->post('video_desc'),'short_desc'=>$this->input->post('short_desc'),'date_time'=>$date_time);
			$config['upload_path'] = 'public/uploads/contents';
			$config['allowed_types'] = 'doc|docx|xls|xlsx|ppt|pptx|jpg|gif|png';
			$this->load->library('upload', $config);
			if($this->upload->do_upload('banner_image'))
			...
			if($loginid){
             ...
             if($this->input->post('category_id')){ 
                if($category ==''){$this->db->where("brand_id = '$id'",null,false);}
                else{$this->db->where("category_id not in ($category) and brand_id = '$id'",null,false);} 
                $this->db->delete('brand_categories');
                foreach($this->input->post('category_id') as $category):if($category>0){
                    if($this->testimonial_model->iscatexists(array('category_id'=>$category,'brand_id'=>$id)))
                    { 
                        //$this->db->update('brand_categories',array('category_title'=>$title,'catslug'=>$slug,'category_order'=>$sort_order[$category]),array('brand_id'=>$brandid,'category_id'=>$category));
                    }else{                        
                        $this->db->insert('brand_categories',array('brand_id'=>$id,'category_id'=>$category));
                    }  
                }
                endforeach;
            }              			 
				...
				redirect('admin/brands/lists/'.$return);
			} else {
	...
	function actions()
	{
		...
		redirect('admin/brands/lists/'.$this->input->post('return'));
	}
