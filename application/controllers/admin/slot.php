...
class Slot extends Web_Controller {
	...
	{
		redirect('admin/slot/lists');		
	}
	
	public function lists()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Slots';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/slot/lists/');
		$config['total_rows'] = $this->slot_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->slot_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->slot_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/slot/lists',$content,true);
	...
	function add()
	{
		...
		$this->form_validation->set_rules('rates', 'Rates', '');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/slot/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$descdata=array('title'=>$this->input->post('title'),'day'=>$this->input->post('day'),'rates'=>$this->input->post('rates'),'is_active'=>$this->input->post('status'));
			$insertid=$this->slot_model->insert($maindata,$descdata);
			if($insertid){
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->slot_model->load($id);
			//$edit['categories']=$this->teamcategory_model->get_array();
			$main['content']=$this->load->view('admin/slot/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			...
			if($loginid){
				...
				redirect('admin/slot/lists/'.$return);
			} else {
	...
	function delete($id,$return)
	{
		...
		//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->slot_model->delete($id);
		if($loginid){
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				$loginid=$this->slot_model->update(array(),$data,$id);	
		...
		redirect('admin/slot/lists/'.$this->input->post('return'));
	}
