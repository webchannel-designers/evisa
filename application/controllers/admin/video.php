...
class Video extends Web_Controller {
...

	{
		...
		$config['base_url'] = site_url('admin/video/lists/'.$id);

		$config['total_rows'] = $this->video_model->get_pagination_count(array('category_id'=>$id));
		...
		$content['galleries']=$this->video_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),array('category_id'=>$id));
...

	{
...

		{
			...
			$maindata=array('status'=>$this->input->post('status'),'category_id'=>$id);
...

	{
...

		{
			...
			$edit['gallery']= $this->video_model->load($pid);
			...
			$loginid=$this->video_model->update($maindata,$descdata,$pid);
