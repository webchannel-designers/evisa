...
class Menus extends Web_Controller {
...

	{
		...
		$this->form_validation->set_rules('code', 'Code', 'required|is_unique[menus.code]');
...

	{
...

		{
			...
			$config['allowed_types'] = 'docx|doc|pdf|rtf|txt|png|jpg';
			...
			$descdata=array('class'=>$this->input->post('class'),'name'=>$this->input->post('name'),'attachment'=>$attachment,'link'=>$this->input->post('link'));
...

	{
...

		{
			...
			$descdata=array('class'=>$this->input->post('class'),'name'=>$this->input->post('name'),'link'=>$this->input->post('link'));
