...
class Types extends Web_Controller {
	...
	{
		redirect('admin/types/lists');		
	}
	
	public function lists()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Types';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/types/lists/');
		$config['total_rows'] = $this->types_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->types_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->types_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/types/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/types/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$insertid=$this->types_model->insert($maindata,$descdata);
			if($insertid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Type added Successfully.</p></div>');
				redirect('admin/types/lists');
			} else {
				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Type not added.</p></div>');
				redirect('admin/types/lists');
			}
		}
	}
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($this->upload->do_upload('image'))
			{
				...
				$this->image_lib->clear();
				$this->resizecrop($imagedata,'view_','936','460');
				$gallery= $this->types_model->load($id);
				if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
			}
			$loginid=$this->types_model->update($maindata,$descdata,$id);
			if($loginid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Type updated Successfully.</p></div>');
				redirect('admin/types/lists/'.$return);
			} else {
	...
	function delete($id,$return)
	{
		...
		//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->types_model->delete($id);
		if($loginid){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Type deleted Successfully.</p></div>');
			redirect('admin/types/lists/'.$return);
		} else {
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				$loginid=$this->types_model->update(array(),$data,$id);	
				$flashmsg = 'Type updated Successfully.';	
			endforeach;			
		}
		...
		if(isset($_POST['delete']) && $ids){			
			foreach($ids as $id):
				...
				//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
				$loginid=$this->types_model->delete($id);
				$flashmsg = 'Type deleted Successfully.';				
			endforeach;
		}
		if($loginid){
		...
		redirect('admin/types/lists/'.$this->input->post('return'));
	}
