...
class Enquiry extends Web_Controller {
...

	{
		...
		$config['base_url'] =base_url().'admin/enquiry/lists/';
		...
		$content['enquiries']=$this->enquiry_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),NULL,'added_on DESC');

		$main['content']=$this->load->view('admin/enquiry/lists',$content,true);
...

	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				...
				$loginid=$this->enquiry_model->update($data,array(),$id);				
...
//		}
		...
		redirect('admin/enquiry/lists/'.$this->input->post('return'));
