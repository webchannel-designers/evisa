...
class Galleryservice extends Web_Controller {
...

    {
		...
		$this->load->model('galleryservice_model');	
...

	{
		...
		redirect('admin/galleryservice/add');		
...

	{
		...
		{
			$edit['records']=$this->galleryservice_model->get_array_cond($product_id);		
			$main['content']=$this->load->view('admin/galleryservice/add',$edit,true);
			$this->load->view('admin/main',$main);
			...
			$descdata=array('parent_id'=>$product_id,'image'=>$fileName,'title'=>$this->input->post('title'),'url'=>$this->input->post('url'),'language'=>$this->session->userdata('admin_language'));

			$insertid=$this->galleryservice_model->insert($maindata,$descdata);
            }	
...
//			}			
			...
			if($insertid){
				...
				redirect('admin/galleryservice/add/'.$product_id);
				...
				//$records=$this->galleryservice_model->get_array();
...
//
//		{
//			$edit['records']=$this->galleryservice_model->get_array();			
...
//			}			
			...
			$insertid=$this->galleryservice_model->update($maindata,$descdata,$key['id']);
...

	{
		...
		$loginid=$this->galleryservice_model->delete($id);
	...
	{
		//$gallerydata = $this->galleryservice_model->load($id);
		$this->galleryservice_model->updateall(array('is_default'=>'N'),array('parent_id'=>$reurn));
		$result = $this->galleryservice_model->update(array(),array('is_default'=>$val),$id);
		//if($result){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Default image updated.</p></div>');

			redirect('admin/galleryservice/add/'.$reurn);
