...
class Comments extends Web_Controller {
...

	{
		...
		$config['base_url'] = site_url('admin/comments/lists/'.$id);

		$config['total_rows'] = $this->comments_model->get_pagination_count(array('pid'=>$id));
		...
		$content['contentfields'] = $this->comments_model->get_fields();

		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');

		$content['contentcats'] = $this->newscategory_model->get_array();

		$content['contents']=$this->comments_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),array('pid'=>$id));

		$main['content']=$this->load->view('admin/comments/content/lists',$content,true);
...

	{
...

		{
			...
			$main['content']=$this->load->view('admin/comments/content/add',$add,true);
			...
			$slug=$this->comments_model->create_slug($this->input->post('title'));
			...
			$insertid=$this->comments_model->insert($maindata,$descdata);

			if($insertid){

				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Comments added Successfully.</p></div>');

				redirect('admin/comments/lists');

			} else {

				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Comments not added.</p></div>');
...

	{
...

		{
			...
			$edit['content']= $this->comments_model->load($id);

			//$edit['widgets']=$this->widgets_model->get_rightwidgets();

			$main['content']=$this->load->view('admin/comments/content/edit',$edit,true);
			...
			$contentrow=$this->comments_model->load($id);
...
//			{
//			$slug=$this->comments_model->update_slug($this->input->post('title'),$id);
//			}
//			else
//			{
//			$slug=$this->comments_model->update_slug($this->input->post('slug'),$id);
//			}
			...
			$descdata=array('name'=>$this->input->post('title'),'email'=>$this->input->post('author'),'comments'=>$this->input->post('comments'),'is_active'=>$this->input->post('status'));
			...
			$loginid=$this->comments_model->update($maindata,$descdata,$id);

			if($loginid){

				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Comments updated Successfully.</p></div>');

				redirect('admin/comments/lists/'.$return);

			} else {

				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Comments not updated.</p></div>');
...

	{
		...
		$content['content'] = $this->comments_model->get_row_cond($cond);				
...

	{
		...
		if(isset($_POST['sortsave']) && $this->input->post('sortsave')=='Save' && count($sort_orders)>0){
			...
			$loginid=$this->comments_model->update($data,array(),$contentid);			
...

	{
		...
		$loginid=$this->comments_model->delete($id);

		if($loginid){

			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Comments deleted Successfully.</p></div>');

			redirect('admin/comments/lists/'.$return);

		} else {

			$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Comments not deleted.</p></div>');
...

	{
		...
		if(isset($status) && $ids){
			...
			foreach($ids as $id):
				...
				$loginid=$this->comments_model->update(array(),$data,$id);				
		...
		redirect('admin/comments/lists/'.$this->input->post('return'));
...

	{
		...
		if ($this->comments_model->code_exists($code,$id))
