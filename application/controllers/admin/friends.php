...
class Friends extends Web_Controller {
...

	{
		...
		$config['base_url'] = site_url('admin/friends/lists/');

		$config['total_rows'] = $this->friends_model->get_pagination_count();
		...
		$content['contentfields'] = $this->friends_model->get_fields();

		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');

		$content['contentcats'] = $this->newscategory_model->get_array();

		$content['contents']=$this->friends_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));

		$main['content']=$this->load->view('admin/friends/content/lists',$content,true);
...

	{
...

		{
			...
			$main['content']=$this->load->view('admin/friends/content/add',$add,true);
			...
			$pid=implode(",",$this->input->post('company'));

			$slug=$this->friends_model->create_slug($this->input->post('title'));
			...
			$descdata=array('title'=>$this->input->post('title'),'location'=>$this->input->post('location'),'phone'=>$this->input->post('phone'),'email'=>$this->input->post('email'),'author'=>$this->input->post('author'),'company'=>$pid,'desc'=>$this->input->post('desc'),'image'=>$content_image,'banner_image'=>$banner_image,'date_time'=>$date_time);

			$insertid=$this->friends_model->insert($maindata,$descdata);

			if($insertid){

				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Data added Successfully.</p></div>');
...

	{
...

		{
			...
			$edit['content']= $this->friends_model->load($id);

			//$edit['widgets']=$this->widgets_model->get_rightwidgets();

			$main['content']=$this->load->view('admin/friends/content/edit',$edit,true);
			...
			$contentrow=$this->friends_model->load($id);
			...
			$slug=$this->friends_model->update_slug($this->input->post('title'),$id);
			
			}
			else
			{
				
			$slug=$this->friends_model->update_slug($this->input->post('slug'),$id);
			...
			$descdata=array('title'=>$this->input->post('title'),'location'=>$this->input->post('location'),'phone'=>$this->input->post('phone'),'email'=>$this->input->post('email'),'author'=>$this->input->post('author'),'company'=>$pid,'desc'=>$this->input->post('desc'),'date_time'=>$date_time);
...

	{
		...
		$content['content'] = $this->friends_model->get_row_cond($cond);				
...

	{
		...
		if(isset($_POST['sortsave']) && $this->input->post('sortsave')=='Save' && count($sort_orders)>0){
			...
			$loginid=$this->friends_model->update($data,array(),$contentid);			
...

	{
		...
		$loginid=$this->friends_model->delete($id);
	...
	{ 
        ...
        $loginid=false;
		$sort_orders=$this->input->post('sortorders'); 
			...
			foreach($sort_orders as $id => $sort_order):
				...
				$loginid=$this->friends_model->update($data,array(),$id);
		...
		redirect('admin/friends/lists/'.$this->input->post('return'));
...

	{
		...
		if ($this->friends_model->code_exists($code,$id))
