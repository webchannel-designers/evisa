...
class Makes extends Web_Controller {
	...
	public function lists()
	{
		...
		$config['base_url'] = site_url('admin/makes/lists/');
		$config['total_rows'] = $this->makes_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->makes_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->makes_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/makes/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/makes/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$descdata=array('category_id'=>$this->input->post('category_id'),'title'=>$this->input->post('title'),'is_active'=>$this->input->post('status'));
			$insertid=$this->makes_model->insert($maindata,$descdata);
			if($insertid){
	...
	function delete($id,$return)
	{
		...
		//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->makes_model->delete($id);
		if($loginid){
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				$loginid=$this->makes_model->update(array(),$data,$id);	
		...
		redirect('admin/makes/lists/'.$this->input->post('return'));
	}
