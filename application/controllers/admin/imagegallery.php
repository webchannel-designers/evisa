...
class Imagegallery extends Web_Controller {
...

	{
		...
		redirect('admin/imagegallery/add');		
...

	{
		...
		{
			$edit['records']=$this->imagegallery_model->get_array();			
			$main['content']=$this->load->view('admin/imagegallery/add',$edit,true);
			$this->load->view('admin/main',$main);
...
//			}
			...
			$descdata=array('image'=>$image,'language'=>$this->session->userdata('admin_language'));

			$insertid=$this->imagegallery_model->insert($maindata,$descdata);
			...
			if($insertid){
				...
				//$records=$this->imagegallery_model->get_array();
...
//			}			
			...
			$descdata=array('sort_order'=>$this->input->post('sort_'.$key['id']));

			$insertid=$this->imagegallery_model->update($maindata,$descdata,$key['id']);
...

	{
		...
		$loginid=$this->imagegallery_model->delete($id);
