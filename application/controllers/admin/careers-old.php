...
class Careers extends Web_Controller {
	...
	function rateapplication($id='',$stat='', $type='')

	{
			...
			echo $lid=$this->applications_model->update(array('rating'=>$this->input->post('r1')),array('id'=>$id));
			//$this->db->last_query();
			if($lid)
			{
		...
		if(isset($_POST['search']) && $this->input->post('search')=='Search'){
            ...
            || $this->input->post('fromdate')!='' || $this->input->post('category')!='' || $this->input->post('shortlist_job')!='' || $this->input->post('sortby')!=''){
				...
				$newdata = array(
					   ...
					   'content_rating_to'  => $this->input->post('rating_to'),
