...
class Instructors extends Web_Controller {
	...
	{
		redirect('admin/instructors/lists');		
	}
	...
	public function lists()
	{
		...
		$config['base_url'] = site_url('admin/instructors/lists/');
		$config['total_rows'] = $this->instructors_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['instructors']=$this->instructors_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/instructors/lists',$content,true);
	...
	function add()
	{
		...
		$this->form_validation->set_rules('username', 'Username', 'required|is_unique[instructors.username]');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$content['roles']=$this->instructors_model->get_roles_array();
			
			$content['courses']=$this->instructors_model->get_courses();
			
			$main['content']=$this->load->view('admin/instructors/add',$content,true);
			$this->load->view('admin/main',$main);
		} else {
			...
			$loginid=$this->instructors_model->insert($data);
			
			$email=$this->instructors_model->getemailaction();
			if($loginid){
	...
	function actions()
	{
		...
		if(count($ids)>0){
			foreach($ids as $id):
				...
				$loginid=$this->instructors_model->update1($data,array('id' => $id));				
			endforeach;
			if($loginid){
		...
		redirect('admin/instructors/lists/'.$this->input->post('return'));
	}
	...
	$this->form_validation->set_rules('fname', 'First Name', 'required');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['roles']=$this->instructors_model->get_roles_array();
			$edit['facu']= $this->instructors_model->load($id);
			$edit['courses']=$this->instructors_model->get_courses();
			$main['content']=$this->load->view('admin/instructors/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else { 
	...
	{
		$cond=array('id'=>$id);
		$loginid=$this->instructors_model->delete($cond);
		if($loginid){
	...
	function changepwd($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$main['content']=$this->load->view('admin/instructors/changepwd',$changepwd,true);
			$this->load->view('admin/main',$main);
		} else {
	...
	function code_exists($code)
	{
		...
		if ($this->instructors_model->code_exists($code,$id))
		{
	...
	public function roles()
	{
		...
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/instructors/roles/');
		$content['roles']=$this->admin_model->get_roles_array();
		$main['content']=$this->load->view('admin/instructors/roles',$content,true);
		$this->load->view('admin/main',$main);
	}
	...
	public function permission()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$config['base_url'] = site_url('admin/instructors/permission/');
			...
			endforeach;
			$main['content']=$this->load->view('admin/instructors/permission',$content,true);
			$this->load->view('admin/main',$main);
		}
		if($this->input->post())
		{
			...
			if($loginid){
				...
				redirect('admin/instructors/permission');
			} else {
...
/* End of file instructors.php */
/* Location: ./application/controllers/admin/instructors.php */