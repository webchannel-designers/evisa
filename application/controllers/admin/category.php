...
class Category extends Web_Controller {
	...
	{
		redirect('admin/careers/category/lists');		
	}
	
	public function lists()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Job Category';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/careers/category/lists/');
		$config['total_rows'] = $this->category_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->category_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->category_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/careers/category/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$insertid=$this->category_model->insert($maindata,$descdata);
			if($insertid){
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->category_model->load($id);
	...
	function delete($id,$return)
	{
		...
		//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->category_model->delete($id);
		if($loginid){
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				$loginid=$this->category_model->update(array(),$data,$id);	
		...
		redirect('admin/category/lists/'.$this->input->post('return'));
	}
