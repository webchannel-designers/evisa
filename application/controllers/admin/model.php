...
class Model extends Web_Controller {
	...
	{
		redirect('admin/model/lists');		
	}
	
	public function lists()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Models';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/model/lists/');
		$config['total_rows'] = $this->model_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['galleryfields'] = $this->model_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		$content['gallerycats'] = $this->teamcategory_model->get_array();
		$content['galleries']=$this->model_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/model/lists',$content,true);
	...
	function add()
	{
		...
		{
			$main['page_title']=$this->config->item('site_name').' - Makes';
			$main['header']=$this->adminheader();
			$main['footer']=$this->adminfooter();
			$main['left']=$this->adminleftmenu();
			$add['contentcats'] = $this->makes_model->get_active();
			$add['categories']=$this->teamcategory_model->get_array();
			$main['content']=$this->load->view('admin/model/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$descdata=array('make_id'=>$this->input->post('category_id'),'title'=>$this->input->post('title'),'is_active'=>$this->input->post('status'));
			$insertid=$this->model_model->insert($maindata,$descdata);
			if($insertid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Make added Successfully.</p></div>');
				redirect('admin/model/lists');
			} else {
				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Make not added.</p></div>');
				redirect('admin/model/lists');
			}
		}
	}
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['left']=$this->adminleftmenu();
			$edit['return']=$return;
			$edit['gallery']= $this->model_model->load($id);
			$edit['contentcats'] = $this->makes_model->get_active();
			//$edit['categories']=$this->teamcategory_model->get_array();
			$main['content']=$this->load->view('admin/model/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			...
			if($this->upload->do_upload('image'))
			{
				...
				$this->image_lib->clear();
				$this->resizecrop($imagedata,'view_','936','460');
				$gallery= $this->model_model->load($id);
				if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
			}
			$loginid=$this->model_model->update($maindata,$descdata,$id);
			if($loginid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Make updated Successfully.</p></div>');
				redirect('admin/model/lists/'.$return);
			} else {
				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Make not updated.</p></div>');
				redirect('admin/model/lists/'.$return);
			}
		}
	}
	...
	function delete($id,$return)
	{
		...
		//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
		$loginid=$this->model_model->delete($id);
		if($loginid){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Make deleted Successfully.</p></div>');
			redirect('admin/model/lists/'.$return);
		} else {
			$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Make not deleted.</p></div>');
			redirect('admin/model/lists/'.$return);
		}
	}
	...
	function actions()
	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				$loginid=$this->model_model->update(array(),$data,$id);	
				$flashmsg = 'Make updated Successfully.';	
			endforeach;			
		}
		...
		if(isset($_POST['delete']) && $ids){			
			foreach($ids as $id):
				...
				//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
				$loginid=$this->model_model->delete($id);
				$flashmsg = 'Make deleted Successfully.';				
			endforeach;
		}
		if($loginid){
		...
		redirect('admin/model/lists/'.$this->input->post('return'));
	}
	
	public function categories()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Make Categories';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
