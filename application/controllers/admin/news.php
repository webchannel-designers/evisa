...
class News extends Web_Controller {
...

	{
...

		{
			...
			$descdata=array('title'=>$this->input->post('title'),'desc'=>$this->input->post('desc'),'image'=>$content_image,'banner_image'=>$banner_image,'date_time'=>$date_time);
...

	{
		...
		$content['content'] = $this->news_model->get_row_cond($cond);				
...

	{
		...
		if(isset($_POST['sortsave']) && $this->input->post('sortsave')=='Save' && count($sort_orders)>0){
			...
			$loginid=$this->news_model->update($data,array(),$contentid);			
