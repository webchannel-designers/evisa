...
class Apply extends Web_Controller {
	...
	{
		redirect('admin/apply/lists');		
	}
	
	public function lists()
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Apply Details';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/apply/lists/');
		$config['total_rows'] = $this->apply_model->get_pagination_count();
		$config['per_page'] = '10';
		...
		$content['contentfields'] = $this->apply_model->get_fields();
		$content['sortorders'] = array('asc'=>'Ascending','desc'=>'Descending');
		//$content['contentcats'] = $this->newscategory_model->get_array();
		$content['contents']=$this->apply_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']));
		$main['content']=$this->load->view('admin/apply/content/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/apply/content/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			...
			$descdata=array('title'=>$this->input->post('title'),'desc'=>$this->input->post('desc'),'image'=>$content_image,'date_time'=>$date_time);
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$descdata=array('title'=>$this->input->post('title'),'desc'=>$this->input->post('desc'),'date_time'=>$date_time);
			$config['upload_path'] = 'public/uploads/contents';
			$config['allowed_types'] = 'gif|jpg|png';
			$this->load->library('upload', $config);
			if($this->upload->do_upload('banner_image'))
	...
	{
		$loginid=$this->apply_model->delete($id);
		if($loginid){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Request deleted Successfully.</p></div>');
			redirect('admin/apply/lists/'.$return);
		} else {
			$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Request not deleted.</p></div>');
			redirect('admin/apply/lists/'.$return);
		}
	}
