...
class Endorsement extends Web_Controller {
...

	{
		...
		redirect('admin/endorsement/lists');		
...

	{
		...
		$config['base_url'] =base_url().'admin/endorsement/lists/';

		$config['total_rows'] = $this->endorsement_model->get_pagination_count();
		...
		$content['enquiries']=$this->endorsement_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),NULL,'added_on DESC');

		$main['content']=$this->load->view('admin/endorsement/lists',$content,true);
...

	{
...

		{
			...
			$edit['enquiry']= $this->endorsement_model->load($id);

			$main['content']=$this->load->view('admin/endorsement/edit',$edit,true);
...
//			}
...

	{
		...
		$loginid=$this->endorsement_model->delete($id);
...

	{
		...
		if(isset($status) && isset($_POST['id'])){
			...
			foreach($ids as $id):
				...
				$loginid=$this->endorsement_model->update($data,array(),$id);				
...
//		}
		...
		redirect('admin/endorsement/lists/'.$this->input->post('return'));
