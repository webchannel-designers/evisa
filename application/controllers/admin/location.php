...
class Location extends Web_Controller {
	...
	public function lists()
	{
		...
		$main['page_title']=$this->config->item('site_name').' - Location';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
		$config['base_url'] = site_url('admin/location/lists/');
		...
		$main['content']=$this->load->view('admin/location/lists',$content,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/location/add',$add,true);
			$this->load->view('admin/main',$main);
		} else {
			$image='';
			...
			$descdata=array('title'=>$this->input->post('title'),'is_active'=>$this->input->post('status'));
			$insertid=$this->location_model->insert($maindata,$descdata);
			if($insertid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Location added Successfully.</p></div>');
				redirect('admin/location/lists');
			} else {
				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Location not added.</p></div>');
				redirect('admin/location/lists');
			}
		}
	}
	...
	function edit($id,$return)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$main['content']=$this->load->view('admin/location/edit',$edit,true);
			$this->load->view('admin/main',$main);
		} else {
			...
			if($loginid){
				$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Location updated Successfully.</p></div>');
				redirect('admin/location/lists/'.$return);
			} else {
				$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Location not updated.</p></div>');
				redirect('admin/location/lists/'.$return);
			}
		}
	}
	...
	function delete($id,$return)
	{
		...
		if($loginid){
			$this->session->set_flashdata('message', '<div class="n_ok flash_messages"><p>Location deleted Successfully.</p></div>');
			redirect('admin/location/lists/'.$return);
		} else {
			$this->session->set_flashdata('message', '<div class="n_error flash_messages"><p>Error!! - Location not deleted.</p></div>');
			redirect('admin/location/lists/'.$return);
		}
	}
	...
	function actions()
	{
		...
		if(isset($_POST['delete']) && $ids){			
			foreach($ids as $id):
				...
				//if($gallery->image!='' && file_exists('public/uploads/gallery/'.$gallery->image)){ unlink('public/uploads/gallery/'.$gallery->image); }
				$loginid=$this->location_model->delete($id);
				$flashmsg = 'Location deleted Successfully.';				
			endforeach;
		}
		if($loginid){
	...
	{
		$this->load->library('pagination');
		$main['page_title']=$this->config->item('site_name').' - Location Categories';
		$main['header']=$this->adminheader();
		$main['footer']=$this->adminfooter();
		$main['left']=$this->adminleftmenu();
