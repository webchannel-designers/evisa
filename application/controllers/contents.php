...
class Contents extends Visafront_Controller {
    ...
    public function team($slug=''){ 
        ...
        //$main['team']=$team=$this->teams_model->get_team(array('slug'=>$slug)); 
        $main['teams']=$this->team_model->get_active('',"FIELD(id,'$slug') DESC,sort_order ASC");
        $this->load->view('frontend/contents/team',$main);  
    }
...

	{
		...
		//print_r($this->pagebannners);
		//$lists['jobs']=$this->jobs_model->get_active();
		
		 $lists['teams']=$this->team_model->get_active();//(array('category_id'=>1));
...
//
...

	{
		...
		$pagemeta=$this->pages_model->get_row_cond(array('key'=>$catcontent->slug));
		
		if($contents->banner_image !="")
		{
		$this->pagebannner=base_url('public/uploads/contents/'.$contents->banner_image); 
		}elseif(isset($pagemeta->banner_image))
		{
