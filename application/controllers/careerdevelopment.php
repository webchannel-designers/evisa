...
class Careerdevelopment extends Ecsfront_Controller {
...

	{
		...
		$pagemeta=$this->pages_model->get_row_cond(array('key'=>'career-development'));
		...
		$config['base_url'] = base_url($this->session->userdata('front_language').'/careerdevelopment/lists/');
		...
		$lists['pagelists']=$this->testimonial_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),NULL,'date_time DESC');
...
else
{
...

	{
		...
		if($news->banner_image!=''){$this->pagebannner=base_url('public/uploads/contents/'.$news->banner_image); }
...
//		$this->currentparentmenu=$currentmenus[1];
		...
		$main['images'] = $this->gallery_model->get_array_cond2($news->id);
