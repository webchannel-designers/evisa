...
class Friends extends Pianofront_Controller {
...
//		if($catcontent->keywords!=''){$this->keys=$catcontent->keywords; }	
		...
		$pagemeta=$this->pages_model->get_row_cond(array('key'=>'friends'));
...
//		$this->breadcrumbarr=array('/'=>$this->alphasettings['BREADCRUMB_START'],site_url('news/lists/'.$catcontent->slug) => $catcontent->name);
		...
		$config['base_url'] = base_url($this->session->userdata('front_language').'/friends/lists?');
...
//
		//$lists['pagelists']=$this->friends_model->get_catnews($id);
		
		$lists['pagelists']=$this->friends_model->get_pagination($config['per_page'],$offset,NULL,'date_time DESC');;
		//$lists['pagelists']=$this->friends_model->get_catnews($id);
		
		//$lists['news']=$this->friends_model->get_array_limit(6);
...
//		$lists['photos'] = $photos;//$galleryphotos;
		$frontcontent=$this->load->view('frontend/testimonial/lists',$lists,true);
	...
	{
 		$this->load->model('frontend/friends_model');
		$this->load->model('frontend/galleryfriends_model');
		//$this->outputCache();
		if($id==''){ redirect('home'); }
		$news=$this->friends_model->get_row_cond(array('slug'=>$id));
		$this->section='news';
...
//		$this->currentparentmenu=$currentmenus[1];
		...
		$main['images'] = $this->galleryfriends_model->get_array_cond2($news->id);

		
		$main['news']=$this->friends_model->get_array_limit(6);
		$frontcontent=$this->load->view('frontend/testimonial/view',$main,true);
		//$main['contents']=$this->frontcontent($frontcontent,true);
