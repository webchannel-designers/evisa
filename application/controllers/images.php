...
class Images extends Pianofront_Controller {
...

	{
		...
		$pagemeta=$this->pages_model->get_row_cond(array('key'=>'promotions'));
...

	{
		...
		$frontcontent=$this->load->view('frontend/images/view',$lists,true);
...

	{
		...
		$lists['gallery'] = $gallery = $this->newslettergallery_model->get_row_cond(array('slug'=>$id),'row'); 
		$gallerycat=$cat =$this->newslettergallerycategory_model->get_row_cond(array('id'=>$gallery->category_id));	
		...
		$this->pagetitle = "Gallery";
		if($gallery->title!=''){$this->pagetitle=$gallery->title;}

		if($gallery->short_desc!=''){$this->desc=$gallery->short_desc; }
		...
		$frontcontent=$this->load->view('frontend/gallery/detail',$lists,true);
