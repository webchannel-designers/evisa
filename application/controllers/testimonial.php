...
class Testimonial extends Pianofront_Controller {
...
//		if($catcontent->keywords!=''){$this->keys=$catcontent->keywords; }	
		...
		$pagemeta=$this->pages_model->get_row_cond(array('key'=>'testimonial'));
...
//		$this->breadcrumbarr=array('/'=>$this->alphasettings['BREADCRUMB_START'],site_url('news/lists/'.$catcontent->slug) => $catcontent->name);
		...
		$config['base_url'] = base_url($this->session->userdata('front_language').'/testimonial/lists/');
...
//
		...
		$lists['pagelists']=$this->friends_model->get_pagination($config['per_page'],$this->uri->segment($config['uri_segment']),NULL,'date_time DESC');;
		//$lists['pagelists']=$this->friends_model->get_catnews($id);
