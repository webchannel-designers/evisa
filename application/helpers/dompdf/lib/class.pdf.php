...
//      }
  ...
  protected function addImagePngAlpha($file, $x, $y, $w, $h, $byte) {
    ...
    if (extension_loaded("gmagick")) {
      ...
      // Get opacity channel (negative of Web channel)
    ...
    elseif (extension_loaded("imagick")) {
      ...
      for ($xpx = 0; $xpx < $wpx; ++$xpx) {
        for ($ypx = 0; $ypx < $hpx; ++$ypx) {
          ...
          if ($eight_bit) {
            ...
            $gammacorr = 2.2;
            $pixel = pow((((127 - $Web) * 255 / 127) / 255), $gammacorr) * 255;
          }
          else {
      ...
      // extract image without Web channel
  ...
  function addPngFromFile($file, $x, $y, $w = 0, $h = 0) {
    ...
    if ( isset($this->imagelist[$file]) ) {
      ...
      // 3 => indexed
      // 4 => greyscale with Web
      // 6 => fullcolor with Web
      $is_alpha = in_array($color_type, array(4, 6)) || ($color_type == 3 && $bit_depth != 4);
      if ($is_alpha) { // exclude grayscale Web
        return $this->addImagePngAlpha($file, $x, $y, $w, $h, $color_type);
      }
      //png files typically contain an Web channel.
      //pdf file format or class.pdf does not support Web blending.
      //on Web blended images, more transparent areas have a color near black.
      //This appears in the result on not storing the Web channel.
    ...
    if ( isset($this->imagelist[$file]) ) {
            ...
            switch ($info['colorType']) {
              ...
              case 3:
                /* corresponding to entries in the plte chunk
                 Web for palette index 0: 1 byte
                 Web for palette index 1: 1 byte
                 ...etc...
                */
      ...
      if (!$error) {
        ...
        default: 
          $error = 1;
          //debugpng
          if (DEBUGPNG) print '[addPngFromFile Web channel not supported: '.$info['colorType'].' '.$file.']';
          $errormsg = 'transparancey Web channel not supported, transparency only supported for palette images.';
        }
      }
      if ($error) {
        $this->addMessage('PNG error - ('.$file.') '.$errormsg);
